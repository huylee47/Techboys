                            ...
                            @foreach ($loadAll as $key => $blog)
                                <tr>
                                    ...
                                    <td class="text-center"><img
                                            src="{{ url('admin/assets/images/blog') . '/' . $blog->image }}" alt=""
                                            style="width: 150px; height: auto;"></td>
                                    <td>{{$blog->published_at }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.blogs.edit',['id'=>$blog['id']]) }}" class="btn btn-info">Sửa</a>
                                        <a href="{{ route('admin.blogs.destroy',['id'=>$blog['id']]) }}" class="btn btn-warning" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">Xóa</a>
                                    </td>
                                </tr>
